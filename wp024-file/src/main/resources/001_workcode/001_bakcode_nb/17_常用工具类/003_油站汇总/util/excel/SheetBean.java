...
public class SheetBean {
     ...
     * 创建一个Sheet对象
     *
     * @param excelBean 所属Excel对象
     * @param sheet     poi原始的Sheet对象
     */
     ...
     * @return
     */
    public List<Object> getRowContent(int rowNum) {
     ...
     * @param style  样式
     */
    public void setRowStyle(int rowNum, CellStyle style) {
        Row row = sheet.getRow(rowNum);
        if (row != null) {
            for (int i = 0; i < row.getLastCellNum(); i++) {
     ...
     * @return
     */
    private Cell getOrCreateCell(int rowNum, int colNum) {
        Row row = sheet.getRow(rowNum);
        if (row == null) {
